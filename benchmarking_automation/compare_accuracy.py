...
def compare(qa_all, pipeline_index):
        ...
        if matching_pipe:
            ...
            if pipe_clean and qa_clean:
                if best_score >= 0.95:
                    ...
                    content_verdict = "PARTIAL (>=70%)"
                elif best_score >= 0.5:
                    content_verdict = "LOW SIMILARITY"
                else:
                    content_verdict = "MISMATCH"
            elif not pipe_clean and not qa_clean:
                content_verdict = "BOTH EMPTY"
            elif not pipe_clean:
                content_verdict = "MISSING IN PIPELINE"
            elif not qa_clean:
                content_verdict = "QA EMPTY / PIPELINE HAS CONTENT"
            else:
    ...
    content_qa_empty = sum(1 for r in results if r["content_match"] == "QA EMPTY / PIPELINE HAS CONTENT")
    both_empty = sum(1 for r in results if r["content_match"] == "BOTH EMPTY")
    ...
    summary = {
        ...
        "unique_coverage_rate": unique_coverage_rate,
        ...
        "content_high_partial": content_high_partial,
        "content_low": content_low,
        "content_mismatch": content_mismatch,
        "content_missing_in_pipeline": content_missing,
        "content_qa_empty": content_qa_empty,
        "both_empty": both_empty,
        "content_acceptable": content_acceptable,
        "content_acceptable_rate": content_acceptable_rate,
        "exact_match_rate": round(content_match_count / content_compared * 100, 2) if content_compared else 0,
    }
...
def write_output(results, summary):
    ...
    items = [
        ...
        ("Low Similarity (>=50% sim)", summary["content_low"]),
        ("MISMATCH", summary["content_mismatch"]),
        ("QA empty / Pipeline has content", summary["content_qa_empty"]),
        ("Pipeline empty", summary["content_missing_in_pipeline"]),
        ("Both empty", summary["both_empty"]),
    ...
    print(f"  Low Similarity (>=50%):  {summary['content_low']}")
    print(f"  MISMATCH:                {summary['content_mismatch']}")
    print(f"  QA empty / Pipe has:     {summary['content_qa_empty']}")
    print(f"  Pipeline empty:          {summary['content_missing_in_pipeline']}")
    print(f"  Both empty:              {summary['both_empty']}")
    print(f"")
    print(f"  Exact match rate:        {summary['exact_match_rate']}%")
    print(f"  Acceptable (>=70% sim):  {summary['content_acceptable']}/{summary['content_compared']} ({summary['content_acceptable_rate']}%)")
    print("=" * 65)
